...
-- Если база уже существует — удаляем её (осторожно, это сотрет все данные!)
IF EXISTS (SELECT * FROM sys.databases WHERE name = 'Staff_DB')
BEGIN
    ALTER DATABASE Staff_DB SET SINGLE_USER WITH ROLLBACK IMMEDIATE;
    DROP DATABASE Staff_DB;
END
GO

-- Создаем базу данных Staff_DB
CREATE DATABASE Staff_DB;
GO

-- Переключаемся на работу с Staff_DB
USE Staff_DB;
GO
...
-- !!! из-за отсутствия необходимых процедур, менять ЗП сотрудника можно только раз в день и вчерашним числом !!!
